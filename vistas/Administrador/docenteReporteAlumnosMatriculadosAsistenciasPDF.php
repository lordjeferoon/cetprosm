<?php 
    ...
    if(isset($_POST['create_pdf'])){
        ...
        $notas = "SELECT * FROM asistencias WHERE ID_ASIGNACION=$idAsignacion";   
        ...
        foreach ($notas as $nota){
            ...
            $idNota=$nota['ID_ASISTENCIA'];
            $notasDetalle = "SELECT * FROM asistencia_detalle WHERE ID_ASISTENCIA=$idNota ORDER BY FECHA ASC";   
            ...
            foreach ($matriculas as $matricula) {
                ...
                if($matricula['ANIO']==$anioActual){
                    ...
                    foreach ($alumnos as $alumno){
                        ...
                        $notas = "SELECT * FROM asistencias WHERE ID_ASIGNACION=$idAsignacion AND ID_ALUMNO=$idAlumno";   
                        $notas=$mysqli->query($notas);
                        ...
                        foreach ($notas as $nota) {
                            ...
                            $idNota=$nota['ID_ASISTENCIA'];
                            $detalles = "SELECT * FROM asistencia_detalle WHERE ID_ASISTENCIA=$idNota ORDER BY FECHA ASC";   
                            $detalles=$mysqli->query($detalles);

                            foreach ($detalles as $detalle) {
                                $tabla.='
                                    <td width="4%">'.$detalle['ASISTENCIA'].'</td>
                                ';
                                $i++;
                                $width=$width+4;
                            } 
                        ...
                        if($i==0){
                            $tabla.='
                                    <td width="61%"  align="center">NO HAY ASISTENCIAS REGISTRADAS</td>
                                </tr>
                            ';
                        }
                        else{
